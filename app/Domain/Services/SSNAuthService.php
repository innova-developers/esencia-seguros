    ...
    private function realAuthentication(string $username, string $cia, string $password): ?array
    {
        try {
            ...
            if ($certPath && file_exists(base_path($certPath))) {
                ...
                Log::info('Usando bundle de certificados SSN para autenticación', [
    ...
    private function createCertificateBundle(string $ssnCertPath): string
    {
        ...
        foreach ($systemCaPaths as $systemPath) {
            if (file_exists($systemPath)) {
                ...
                $systemCaFound = true;
                Log::info('Usando certificados del sistema para autenticación', ['path' => $systemPath]);
                break;
            }
        }
        ...
        if (!$systemCaFound) {
            Log::warning('No se encontraron certificados del sistema para autenticación, usando solo certificado SSN');
        }
        ...
        Log::info('Bundle de certificados SSN creado para autenticación', [
            'bundle_path' => $bundlePath,
            'size' => strlen($bundle),
